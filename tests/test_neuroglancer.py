...
def test_neuroglancer(mesh, tmp_path):
    def writer(*args, **kwargs):
        return meshlane.neuroglancer.write(*args, **kwargs)

    # 32bit only
    helpers.write_read(tmp_path, writer, meshlane.neuroglancer.read, mesh, 1.0e-8)
...
def test_reference_file(filename, ref_sum, ref_num_cells):
    ...
    mesh = meshlane.read(filename, "neuroglancer")
    tol = 1.0e-5
