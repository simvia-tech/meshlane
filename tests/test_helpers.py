...
def test_read_str():
    meshlane.read(str(OBJ_PATH))


def test_read_pathlike():
    meshlane.read(OBJ_PATH)
...
def test_write_buffer(mesh, tmpdir):
    ...
    with open(tmp_path, "w") as f:
        meshlane.write(f, mesh, "ply")
    assert Path(tmp_path).is_file()


def test_msh_format_selection_for_med_data():
    from meshlane._helpers import _pick_best_format
    ...
    mesh3 = meshlane.Mesh(points, cells)
