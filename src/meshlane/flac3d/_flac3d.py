...
def read_buffer(f, binary):
    ...
    # global IDs to the running index (0, 1,..., n) that's used in meshlane.
    if len(f_cell_ids) > 0:
...
def write(filename, mesh: Mesh, float_fmt: str = ".16e", binary: bool = False):
    ...
    with open_file(filename, mode) as f:
        if binary:
            ...
            f.write(f"* FLAC3D grid produced by meshlane v{version}\n")
...
def _translate_zcells(points, cells):
    """Reorder meshlane cells to FLAC3D zones.
...
def _translate_fcells(cells):
    """Reorder meshlane cells to FLAC3D faces."""
    faces = []
    for cell_block in cells:
...
def _translate_groups(cells, cell_data, field_data, flag):
    """Convert meshlane cell_data to FLAC3D groups."""
    dim = np.concatenate(
