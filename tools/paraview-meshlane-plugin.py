...
paraview_plugin_version = meshlane.__version__
vtk_to_meshio_type = meshlane._vtk_common.vtk_to_meshio_type
meshio_to_vtk_type = meshlane._vtk_common.meshio_to_vtk_type
meshio_input_filetypes = list(meshlane._helpers.reader_map.keys())
meshio_extensions = [ext[1:] for ext in meshlane.extension_to_filetypes.keys()]
meshio_input_filetypes = ["automatic"] + meshio_input_filetypes


@smproxy.reader(
    name="meshlane reader",
    extensions=meshio_extensions,
    file_description="meshlane-supported files",
...
class MeshioReader(VTKPythonAlgorithmBase):
    ...
    @smdomain.filelist()
    @smhint.filechooser(
        extensions=meshio_extensions, file_description="meshlane-supported files"
    )
    ...
    def RequestData(self, request, inInfoVec, outInfoVec):
        ...
        # Use meshlane to read the mesh
        mesh = meshlane.read(self._filename, self._file_format)
...
class MeshioWriter(VTKPythonAlgorithmBase):
    ...
    def RequestData(self, request, inInfoVec, outInfoVec):
        ...
        cells = [meshlane.CellBlock(key, cells_dict[key]) for key in cells_dict]
        ...
        # Use meshlane to write mesh
        meshlane.write_points_cells(
            self._filename,
            points,
            cells,
